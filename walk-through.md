...
This tutorial is a part of a larger Flat Data project created by [GitHub OCTO](https://octo.github.com/). Read more about the project [here](https://octo.github.com/projects/flat-data).
...
3. **Add the following code to the flat.yaml file:**
	...
	on:
	  push:
	    paths:
        # Only run a new workflow every time this file (flat.yml) file has changed
...
1. **Launch a GitHub Action workflow:** Without doing anything, our Action will run at 12:00 p.m. (noon) every day and fetch any new Bitcoin price data. For the sake of this tutorial, we’ll also trigger it manually so you can see the result instantly. 
...
3. **Check the commit history:** If you check the commit history for the repository, you’ll see that Flat Action logs when a particular file changed and was committed to your repository.
    ...
    At this point you’ve used Flat Data to get data on a schedule (every day ay 12:00 p.m.) into your repo. Congrats! But Flat Data can do a bit more. Read on to see how you can add an extra step to process your data in more advanced ways. 
...
3. **Add a postprocess parameter to flat.yaml:** Go back to the flat.yaml file and click on the pencil icon to edit it. Add the `postprocess` line below downloaded_filename. This tells the Flat Action that we want to pass the downloaded file through a script that will process it and do additional work.
	...
	- name: Fetch data 
	        ...
	        with:
	          http_url: https://api.coindesk.com/v2/bpi/currentprice.json
            # The http_url gets saved and renamed in our repository as btc-price.json
	          downloaded_filename: btc-price.json
